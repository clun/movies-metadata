...
public class MovieMetadata {
    ...
    private void parseFile() {
        ...
        for (General gKey : General.values()) {
            String val = info.get(MediaInfo.StreamKind.General, 0, gKey.getKey(),  
        ...
        for (Audio aKey : Audio.values()) {
            String val = info.get(MediaInfo.StreamKind.Audio, 0, aKey.getKey(), 
                    MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);
            if (val != null && !val.isEmpty()) {
        ...
        for (Video vKey : Video.values()) {
            String val = info.get(MediaInfo.StreamKind.Video, 0, vKey.getKey(),  
                    MediaInfo.InfoKind.Text, MediaInfo.InfoKind.Name);
            if (val != null && !val.isEmpty()) {
