...
public class DocuDbDao {
    ...
    public List < String > getListOfGenres() {
        if (genres.isEmpty()) {
            genres.addAll(getJdbcTemplate().query("SELECT NOM FROM ref_genre WHERE PARENT IS NULL order by NOM ASC", new SingleColumnRowMapper<String>()));
        }
        return genres;
    }
